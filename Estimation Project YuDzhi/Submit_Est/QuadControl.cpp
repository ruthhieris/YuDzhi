...
V3F QuadControl::LateralPositionControl(V3F posCmd, V3F velCmd, V3F pos, V3F vel, V3F accelCmdFF)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    ...
    V3F vel_term = velCmd + kpPosXY * (posCmd - pos);
    if (vel_term.mag() > maxSpeedXY)
    {
        vel_term *= maxSpeedXY / vel_term.mag();
    }
    
    accelCmd += kpVelXY * (vel_term - vel);
...
float QuadControl::YawControl(float yawCmd, float yaw)
{
 ...
 yaw_err = fmodf(yaw_err, 2.f * F_PI);
 if (yaw_err > F_PI)
 {
     yaw_err -= 2.f * F_PI;
 }
