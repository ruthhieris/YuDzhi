...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // make sure you comment it out when you add your own code -- otherwise e.g. you might integrate yaw twice
    ...
    Quaternion<float> attEst = EstimatedAttitude(); //Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
  //Quaternion<float> predictedQt = attEst.IntegrateBodyRate_fast(gyro.x, gyro.y, gyro.z, dtIMU / 2.f);
  Quaternion<float> predictedQt = attEst.IntegrateBodyRate(gyro, dtIMU);
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
	...
	//UpdateFromIMU(accel_Ifr, gyro);
    predictedState[0] = predictedState[0] + predictedState[3] * dt; // x_pred = x_t + xdot_t * dt
    predictedState[1] = predictedState[1] + predictedState[4] * dt; // y_pred = y_t + ydot_t * dt
    predictedState[2] = predictedState[2] + predictedState[5] * dt; // z_pred = z_t + zdot_t * dt
    predictedState[3] = predictedState[3] + accel_Ifr.x * dt; // xdot_pred = xdot_t + xddot_t * dt
    predictedState[4] = predictedState[4] + accel_Ifr.y * dt; // ydot_pred = ydot_t + yddot_t * dt
    predictedState[5] = predictedState[5] + (accel_Ifr.z - CONST_GRAVITY) * dt; // zdot_pred = zdot_t + zddot_t * dt
    //predictedState[6] = predictedState[6] = currentState[6] = ekfState[6];
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  assert(ekfCov.cols() == gPrimeT.rows());
  assert(QUAD_EKF_NUM_STATES == gPrime.cols());
